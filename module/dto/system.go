...
type SystemUpdateDto struct {
	...
	RegisterQuota      *int    `json:"registerQuota" validate:"omitempty,gte=0,lte=1024000,validatePositiveInt"` // 注册用户默认配额 单位/MB
	RegisterExpireDays *uint   `json:"registerExpireDays" validate:"omitempty,gte=0,lte=365"`                    // 注册用户过期天数 单位/天
	ExpireWarnEnable   *uint   `json:"expireWarnEnable" redis:"expireWarnEnable" validate:"required,oneof=0 1"`
	ExpireWarnDay      *uint   `json:"expireWarnDay" redis:"expireWarnDay" validate:"omitempty,gte=0,lte=365"`
	EmailEnable        *uint   `json:"emailEnable" redis:"emailEnable" validate:"required,oneof=0 1"`
	EmailHost          *string `json:"emailHost" form:"emailHost" validate:"omitempty,ip|fqdn,min=4,max=64"`
	EmailPort          *uint   `json:"emailPort" form:"emailPort" validate:"omitempty,validatePort"`
