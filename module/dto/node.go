...
type NodeQRCodeDto struct {
	Name            *string `json:"name" form:"name" validate:"omitempty,min=2,max=20"`
	...
	WebsocketPath   *string `json:"websocketPath" validate:"omitempty,min=2,max=32"`
	SsEnable        *uint   `json:"ssEnable" validate:"omitempty,oneof=0 1"`
	SsMethod        *string `json:"ssMethod" validate:"omitempty,min=2,max=16"`
	SsPassword      *string `json:"ssPassword" validate:"omitempty,min=2,max=32"`
}
...
type NodeUpdateDto struct {
	RequiredIdDto
	Name            *string `json:"name" form:"name" validate:"omitempty,min=2,max=20"`
	Ip              *string `json:"ip" form:"ip" validate:"omitempty,ip|fqdn,min=4,max=64"`
	Port            *uint   `json:"port" form:"port" validate:"omitempty,validatePort"`
